...
if __name__ == '__main__':
    data = pd.read_csv('repo/datasets/miplib_cut_dynamic/0328_allin1.csv')
    trains, vals, tests = cross_validation_split(data,  k=5, seed=42, use_val=False)
    # if not os.path.exists('repo/labels/mipcut_bin_5fold'):
    #     os.mkdir('repo/labels/mipcut_bin_5fold')
    root = 'repo/datasets/miplib_cut_dynamic/labels/kfold/'
    if not os.path.exists(root+'labels/kfold/'):
        os.makedirs(root+'labels/kfold/')
    for i in range(5):
        #print(f'Fold {i+1} train shape: {trains[i].shape}, val shape: {vals[i].shape}, test shape: {tests[i].shape}')
        trains[i].to_csv(root+f'fold_{i+1}_train.csv', index=False)
        #vals[i].to_csv(f'repo/datasets/miplib/labels/kfold/fold_{i+1}_val.csv', index=False)
        tests[i].to_csv(root+f'fold_{i+1}_test.csv', index=False)