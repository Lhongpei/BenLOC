...
def parse_log_file(file_path):
    ...
    data = []
    params_header = []
    headers = ['solfndby'] + params_header + ['Nodes', 'Active', 'LPit/n', 'Depth', 'MDpt', 'IntInf', 'GlbFix', 'GlbRed', '#Cuts', 'MaxEff', '#MCP', '#Sepa', '#Nnz', '#SB', '#Conf', 'Local Bound', 'BestBound', 'BestSolution', 'Gap', 'Progr.', 'Time', 'Work']
    row = []
    log_starts = False
    pattern = re.compile(r'(\w*|\**)\s+(\d+)\s+(\d+)\s+([\d.e+-]*|--)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+([\d.e+-]*|--)\s+([\d.e+-]*|--)\s+([\d.e+-]*|--)\s+([\d.e+-]*\w+|--)\s+([\d.e+-]*|--)\s+([\d.e+-]*|--)\s+([\d.e+-]*|--)\s+([\d.e+-]*|--)\s+([\d.e+-]*|--)\s+([\d.e+%Inf]*|--)\s+([\d.e+%Inf]*|--)\s+([\d.]+)s\s+([\d.]+)s')
    for line in lines:
        if log_starts:
...
if __name__ == '__main__':
    ...
    log_dir = 'copt-log/copt-miplib-treecut-200x2ext/log-treecut-def'
    ...
    df.to_csv('parsed_log-treecut-200x2ext-def.csv', index=False)
