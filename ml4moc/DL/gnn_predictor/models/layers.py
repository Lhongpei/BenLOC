...
class predictMLP(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim_list, num_layers=1, BN = True):
        ...
        if BN:
            for i in range(num_layers):
                self.mlp.append(nn.Sequential(nn.Linear(hidden_dim_list[i], hidden_dim_list[i+1]),
