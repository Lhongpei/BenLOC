...
from BenLOC.DL.gnn_predictor.PyGmodel.tasks_pyg import (RegrNetPool, RankListNetPool, RankListNetNode)
...
if __name__ == '__main__':
    ...
    predictNet = RegrNetPool if not ListWise else RankListNetNode
    dataset = RegrSolTimeHeteroDataset if not ListWise else RankListConfigNodesDataset
    ...
    for fold_step in range(1, fold + 1):
        try:
            ...
            for ep in tqdm(range(epoch),desc=f'Fold_{fold_step} Train Epoch:'):
                cal_improve = True
                train_result = forward_loop(Loss = Loss, model=model, dataloader=train_dataloader, report_Dict=report_Dict, optimizer=optimizer, scheduler=scheduler, device=device, use_wandb=use_wandb, ep=ep, fold=fold_step, 
                ...
                valid_result = forward_loop(Loss = Loss, model=model, dataloader=valid_dataloader, report_Dict=report_Dict, optimizer=optimizer, scheduler=scheduler, device=device, use_wandb=use_wandb, ep=ep, fold=fold_step, 
                ...
                test_result = forward_loop(Loss = Loss, model=model, dataloader=test_dataloader, report_Dict=report_Dict, optimizer=optimizer, scheduler=scheduler, device=device, use_wandb=use_wandb, ep=ep, fold=fold_step, 
