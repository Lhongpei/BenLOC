...
from torch_geometric.nn import (GATConv, GINEConv, GraphNorm, JumpingKnowledge,
                                global_mean_pool)
...
class Encoder(nn.Module):
    def __init__(self, input_dim_xs, input_dim_xt, input_dim_edge, num_layers, hidden_dim, mlp_hidden_dim, enc_dim, jk_mode='cat'):
        ...
        self.mlp_xs_mu = nn.Sequential(nn.Linear(jk_dim, mlp_hidden_dim),
                                       nn.BatchNorm1d(mlp_hidden_dim),
                                       nn.ReLU())
        self.mlp_xt_mu = nn.Sequential(nn.Linear(jk_dim, mlp_hidden_dim),
                                       nn.BatchNorm1d(mlp_hidden_dim),
                                       nn.ReLU())

        self.mlp_xs_logsigma = nn.Sequential(nn.Linear(jk_dim, mlp_hidden_dim),
                                             nn.BatchNorm1d(mlp_hidden_dim),
                                             nn.ReLU())
        self.mlp_xt_logsigma = nn.Sequential(nn.Linear(jk_dim, mlp_hidden_dim),
                                             nn.BatchNorm1d(mlp_hidden_dim),
                                             nn.ReLU())

        self.fc_xs_mu = nn.Linear(mlp_hidden_dim, enc_dim)
        self.fc_xt_mu = nn.Linear(mlp_hidden_dim, enc_dim)
        self.fc_xs_logsigma = nn.Linear(mlp_hidden_dim, enc_dim)
        self.fc_xt_logsigma = nn.Linear(mlp_hidden_dim, enc_dim)

    def reparametrize(self, mu, log_sigma):
        if self.training:
            return mu + torch.randn_like(log_sigma) * torch.exp(torch.clamp(log_sigma, max=20))
        else:
            return mu
...
class Decoder(nn.Module):
    def __init__(self, input_dim_xs, input_dim_xt, input_dim_edge, num_layers, hidden_dim, mlp_hidden_dim, mlp_out_dim, enc_dim, jk_mode='cat'):
        ...
        self.mlp_hxs = nn.Sequential(nn.Linear(jk_dim, mlp_hidden_dim),
                                     nn.BatchNorm1d(mlp_hidden_dim),
                                     nn.ReLU())
        self.mlp_hxt = nn.Sequential(nn.Linear(jk_dim, mlp_hidden_dim),
                                     nn.BatchNorm1d(mlp_hidden_dim),
                                     nn.ReLU())
        ...
        '''self.mlp_degree = nn.Sequential(nn.Linear(mlp_out_dim+1, mlp_hidden_dim),
                                 ...
                                 nn.Sigmoid())'''
        self.mlp_degree = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
        ...
        '''self.mlp_logits = nn.Sequential(nn.Linear(mlp_out_dim+1, mlp_hidden_dim),
                                        ...
                                        nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_weights = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                         nn.BatchNorm1d(hidden_dim),
                                         nn.ReLU(),
                                         nn.Linear(mlp_hidden_dim, 1),
                                         # nn.Sigmoid()
        ...
        self.mlp_hlb_s = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                     nn.BatchNorm1d(hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(mlp_hidden_dim, 1),
                                     # nn.Sigmoid()
        ...
        '''self.mlp_ym = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_hub_s = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                     nn.BatchNorm1d(hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(mlp_hidden_dim, 1),
                                     # nn.Sigmoid()
        ...
        '''self.mlp_s = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_lb_s = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                    nn.BatchNorm1d(hidden_dim),
                                    nn.ReLU(),
                                    nn.Linear(mlp_hidden_dim, 1),
                                    # nn.Sigmoid()
        ...
        '''self.mlp_rm = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_ub_s = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                    nn.BatchNorm1d(hidden_dim),
                                    nn.ReLU(),
                                    nn.Linear(mlp_hidden_dim, 1),
                                    # nn.Sigmoid()
        ...
        '''self.mlp_x = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_hlb_t = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                     nn.BatchNorm1d(hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(mlp_hidden_dim, 1),
                                     # nn.Sigmoid()
        ...
        '''self.mlp_ym = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_hub_t = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                     nn.BatchNorm1d(hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(mlp_hidden_dim, 1),
                                     # nn.Sigmoid()
        ...
        '''self.mlp_yn = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_c = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                   nn.BatchNorm1d(hidden_dim),
                                   nn.ReLU(),
                                   nn.Linear(mlp_hidden_dim, 1),
                                   # nn.Sigmoid()
        ...
        '''self.mlp_s = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_lb_t = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                    nn.BatchNorm1d(hidden_dim),
                                    nn.ReLU(),
                                    nn.Linear(mlp_hidden_dim, 1),
                                    # nn.Sigmoid()
        ...
        '''self.mlp_rm = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_ub_t = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                    nn.BatchNorm1d(hidden_dim),
                                    nn.ReLU(),
                                    nn.Linear(mlp_hidden_dim, 1),
                                    # nn.Sigmoid()
        ...
        '''self.mlp_rn = nn.Sequential(nn.Linear(mlp_out_dim+1, 
                                             ...
                                             nn.Linear(mlp_hidden_dim, 1),
                                 nn.Sigmoid())'''
        self.mlp_t = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                   nn.BatchNorm1d(hidden_dim),
                                   nn.ReLU(),
                                   nn.Linear(mlp_hidden_dim, 1),
                                   # nn.Sigmoid()
        ...
        self.mlp_logits = nn.Sequential(nn.Linear(mlp_out_dim + enc_dim, mlp_hidden_dim),
                                        nn.BatchNorm1d(hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(mlp_hidden_dim, 1),
                                        # nn.Sigmoid()
    ...
    def forward(self, masked_x_s, masked_x_t, masked_edge_index, masked_edge_attr, xs_z, xt_z):
        ...
        for conv_s_t, conv_t_s in zip(self.conv_s_t, self.conv_t_s):
            ...
            x_s_outs.append(new_x_s)
            x_t_outs.append(new_x_t)
            
        masked_x_t = self.jk_x_t(x_t_outs)
        masked_x_s = self.jk_x_s(x_s_outs)
        ...
        # predict s mlp[ht,zt]
        predict_hlb_s = self.mlp_hlb_s(hz_xs)
        # predict hub mlp[ht,zt]
        predict_hub_s = self.mlp_hub_s(hz_xs)
        # predict lb mlp[ht,zt]
        predict_lb_s = self.mlp_lb_s(hz_xs)
        # predict ub mlp[ht,zt]
        predict_ub_s = self.mlp_ub_s(hz_xs)
        # predict hlb mlp[ht,zt]
        predict_hlb_t = self.mlp_hlb_t(hz_xt)
        # predict hub mlp[ht,zt]
        predict_hub_t = self.mlp_hub_t(hz_xt)
        # predict c mlp[hs,zs]
        predict_c = self.mlp_c(hz_xt)
        # predict lb mlp[ht,zt]
        predict_lb_t = self.mlp_lb_t(hz_xt)
        # predict ub mlp[ht,zt]
        predict_ub_t = self.mlp_ub_t(hz_xt)
        # predict t mlp[ht,zt]
        predict_t = self.mlp_t(hz_xt)

        return predict_degree, predict_weights, predict_hlb_s, predict_hub_s, predict_lb_s, predict_ub_s, predict_hlb_t, predict_hub_t, predict_c, predict_lb_t, predict_ub_t, predict_t, predict_logits
