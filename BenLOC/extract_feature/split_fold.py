...
def splitfold(name, time_path, feat_path):
    ...
    single_class_test_mask = np.random.choice(
        [True, False], size=len(single_class_indices)
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="which folder to get the solving time",
    )
    parser.add_argument(
        "--feat_path",
        dest="feat_path",
        type=str,
        default="./data",
        help="which folder to get the features",
    )
