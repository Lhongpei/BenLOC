...
def setup_logger(index, folder='new_label/log'):
...
if __name__ == "__main__":
    space = DesignSpace().parse([
        ...
        {'name':  'min_samples_leaf', 'type': 'int', 'lb': 1, 'ub': 20},
        {'name':'min_samples_split','type': 'int', 'lb': 2, 'ub': 20},
    ...
    for index in ['load_balance','indset']:
        ...
        for i in ind_range:
            ...
            data_train=preprocess(train_file,default)
            data_test=preprocess(test_file,default)
            ...
            folder_path = 'new_label/model_result/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            with open(f'new_label/model_result/model_{index}_fold{i}.pickle', 'wb') as f:
            ...
            min_idx_train = data_train.groupby('File Name')['predict_y'].idxmax()
            result_train = data_train.loc[min_idx_train].reset_index(drop=True)
            min_idx_test = data_test.groupby('File Name')['predict_y'].idxmax()
            ...
            folder_path = 'new_label/stat_result/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            folder_path = 'new_label/pred_result/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            df.to_csv(f"new_label/stat_result/stat_{index}_fold{i}.csv")

            data_test.to_csv(f"new_label/pred_result/pred_{index}_fold{i}_test.csv")

            data_train.to_csv(f"new_label/pred_result/pred_{index}_fold{i}_train.csv")
