...
from DL.models.layers import predictMLP, FourierEncoder
...
class BiparGAT(nn.Module):
    ...
    def __init__(self, config, data):
        ...
        self.convs.append(HeteroConv({
            ('cons', 'to', 'vars'): GATv2Conv((cons_enc_dim, var_enc_dim), self.hidden_dim // self.num_heads, add_self_loops=False, heads=self.num_heads, edge_dim=1),
            ('vars', 'to', 'cons'): GATv2Conv((var_enc_dim, cons_enc_dim), self.hidden_dim // self.num_heads, add_self_loops=False, heads=self.num_heads, edge_dim=1),
        }))
    ...
    def forward(self, heteroData):
        ...
        initial_x_dict = copy.deepcopy(x_dict)
        ...
        for conv in self.convs:
            ...
            x_dict = {key: F.leaky_relu(x + initial_x_dict[key]) for key, x in x_dict.items()}
